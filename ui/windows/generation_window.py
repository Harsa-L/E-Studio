...
from PySide6.QtWidgets import (
    ...
    QMainWindow, QMessageBox, QVBoxLayout, QWidget, QInputDialog,
    QTableWidget, QTableWidgetItem, QComboBox, QDialog, QDialogButtonBox,
)

from core.workflows.generation_session import GenerationSession, TemplateSnapshot
from core.rendering.template_canvas import TemplateCanvas
from core.workflows.data_sources import ImportedRow, WorkbookReadError, read_xlsx
from core.workflows.data_normalization import normalize_row
from core.workflows.binding_profiles import BindingProfile, suggest_mappings
from core.workflows.batch_export import BatchReport, BatchRowResult, export_rows_as_png, export_rows_on_pages, normalize_workbook
from core.workflows.data_sources import ImportIssue
from core.rendering.tier_grouping import TierGroupingConfig, normalize_grouped_articles
from core.workflows.output_service import OutputService
from ui.windows.template_editor_window import TemplateEditorWindow
from core.settings.app_settings import GenerationEditPolicy
from ui.main_window import ExportOptionsDialog, MappingDialog
...
class GenerationWindow(QMainWindow):
    ...
    def __init__(self, snapshot: TemplateSnapshot, parent=None):
        ...
        root = QWidget(self)
        root.setObjectName("generationRoot")
        layout = QVBoxLayout(root)
        layout.setContentsMargins(18, 18, 18, 18)
        layout.setSpacing(14)

        header = FlowLayout(spacing=8)
        title = QLabel(f"Gabarit verrouillé : {snapshot.name}")
        title.setStyleSheet("font-size: 15px; font-weight: 600;")
        header.addWidget(title)

        self.edit_button = make_icon_button(icon_name="pencil", icon_group="editing", label="Modifier le gabarit", size_tier="md")
        self.edit_button.clicked.connect(self.edit_template_requested)
        header.addWidget(self.edit_button)
        self.import_button = make_icon_button(icon_name="file_import", icon_group="files", label="Importer Excel", size_tier="md")
        self.import_button.clicked.connect(self.import_excel)
        header.addWidget(self.import_button)
        self.export_button = make_icon_button(icon_name="download_2", icon_group="files", label="Exporter PNG", size_tier="md")
        self.export_button.setEnabled(False)
        self.export_button.clicked.connect(self.export_rows)
        header.addWidget(self.export_button)
        self.pages_button = make_icon_button(icon_name="layout", icon_group="design", label="Exporter feuilles", size_tier="md")
        self.pages_button.setEnabled(False)
        self.pages_button.clicked.connect(self.export_pages)
        header.addWidget(self.pages_button)
        self.pdf_button = make_icon_button(icon_name="pdf", icon_group="files", label="Exporter PDF", size_tier="md")
        self.pdf_button.setEnabled(False)
        self.pdf_button.clicked.connect(self.export_pdf)
        header.addWidget(self.pdf_button)
        self.print_button = make_icon_button(icon_name="document_3", icon_group="files", label="Imprimer", size_tier="md")
        self.print_button.setEnabled(False)
        self.print_button.clicked.connect(self.print_label)
        header.addWidget(self.print_button)
        self.print_preview_button = make_icon_button(icon_name="fullscreen", icon_group="media", label="Aperçu impression", size_tier="md")
        ...
        self.previous_button = make_icon_button(icon_name="arrow_left", icon_group="arrow", label="Précédent", size_tier="md")
        self.previous_button.setEnabled(False)
        self.previous_button.clicked.connect(self.select_previous)
        header.addWidget(self.previous_button)
        self.next_button = make_icon_button(icon_name="arrow_right", icon_group="arrow", label="Suivant", size_tier="md")
        self.next_button.setEnabled(False)
        self.next_button.clicked.connect(self.select_next)
        header.addWidget(self.next_button)
        self.refresh_preview_button = make_icon_button(icon_name="refresh_1", icon_group="system", label="Actualiser aperçu", size_tier="md")
        self.refresh_preview_button.clicked.connect(self.refresh_preview)
        header.addWidget(self.refresh_preview_button)
        self.save_profile_button = make_icon_button(icon_name="save_2", icon_group="files", label="Enregistrer profil", size_tier="md")
        self.save_profile_button.setEnabled(False)
        self.save_profile_button.clicked.connect(self.save_profile)
        header.addWidget(self.save_profile_button)
        self.load_profile_button = make_icon_button(icon_name="folder_open", icon_group="files", label="Charger profil", size_tier="md")
        self.load_profile_button.clicked.connect(self.load_profile)
        header.addWidget(self.load_profile_button)
        layout.addLayout(header)

        info_label = QLabel("Cette génération utilise une copie immuable du gabarit sélectionné.")
        info_label.setWordWrap(True)
        layout.addWidget(info_label)

        source_header = FlowLayout(spacing=8)
        source_title = QLabel("Données importées et modifiables")
        source_title.setStyleSheet("font-weight: 600;")
        source_header.addWidget(source_title)
        self.add_row_button = make_icon_button(icon_name="add", icon_group="system", label="Ajouter", size_tier="sm")
        ...
        self.remove_row_button = make_icon_button(icon_name="delete", icon_group="system", label="Supprimer", size_tier="sm")
        ...
        self.duplicate_row_button = make_icon_button(icon_name="copy_2", icon_group="files", label="Dupliquer", size_tier="sm")
        ...
        self.reprocess_button = make_icon_button(icon_name="refresh_3", icon_group="system", label="Recalculer", size_tier="sm")
        ...
        self.preview_view.setMinimumWidth(420)
        self.preview_view.setMinimumHeight(320)

        self.rows = QListWidget()
        self.rows.setMinimumHeight(170)
        ...
        preview_title = QLabel("Aperçu du gabarit")
        ...
        data_group_layout.addLayout(source_header)
        data_group_layout.addWidget(self.source_table, 2)
        data_layout.addWidget(data_group)

        rows_group = QWidget()
        rows_group_layout = QVBoxLayout(rows_group)
        rows_group_layout.setContentsMargins(0, 0, 0, 0)
        rows_group_layout.setSpacing(8)
        rows_title = QLabel("Lignes source")
        rows_title.setStyleSheet("font-weight: 600;")
        rows_group_layout.addWidget(rows_title)
        rows_group_layout.addWidget(self.rows, 1)
        data_layout.addWidget(rows_group)

        diagnostics_group = QWidget()
        diagnostics_group_layout = QVBoxLayout(diagnostics_group)
        diagnostics_group_layout.setContentsMargins(0, 0, 0, 0)
        diagnostics_group_layout.setSpacing(8)
        diagnostics_group_layout.addLayout(diagnostics_header)
        diagnostics_group_layout.addWidget(self.diagnostics, 1)
        data_layout.addWidget(diagnostics_group)

        content.addWidget(preview_panel, 2)
        content.addWidget(data_panel, 1)
    ...
    def _apply_session_actions(self):
        decision = self.session.business_decision()
        self.edit_button.setEnabled(decision.can_edit)
        self.import_button.setEnabled(decision.can_import)
        self.export_button.setEnabled(decision.can_export and self.batch_report is not None)
        self.pages_button.setEnabled(decision.can_export and self.batch_report is not None)
        self.pdf_button.setEnabled(decision.can_export and self.batch_report is not None and self.settings.value("allow_pdf_output", True, type=bool))
        self.print_button.setEnabled(decision.can_export and self.batch_report is not None)
        self.print_preview_button.setEnabled(decision.can_preview and self.batch_report is not None)
        self.add_row_button.setEnabled(decision.can_import and self.excel_table is not None)
        self.remove_row_button.setEnabled(decision.can_import and bool(self.source_table.selectedIndexes()))
        self.duplicate_row_button.setEnabled(decision.can_import and self.source_table.currentRow() >= 0)
        self.reprocess_button.setEnabled(decision.can_import and self.excel_table is not None)
        self.save_profile_button.setEnabled(decision.can_import and self.excel_profile is not None)
    ...
    def import_excel(self):
        ...
        try:
            ...
            self.session.transition_to("preparing")
            self._apply_session_actions()
    ...
    def reprocess_source(self):
        ...
        self.session.transition_to("previewing")
        if grouped:
