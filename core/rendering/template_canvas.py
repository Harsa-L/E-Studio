...
from core.rendering.label_items import BaseLabelItem, HazardWarningOverlay
from core.rendering.imposition import ImpositionCalculator
...
class TemplateCanvas(QGraphicsScene):
    ...
    def __init__(self, template: LabelTemplate, scale_px_per_mm: float = 3.78):
        ...
        self._bounds_validation_timer = QTimer(self)
        self._bounds_validation_timer.setSingleShot(True)
        self._bounds_validation_timer.setInterval(60)
        self._bounds_validation_timer.timeout.connect(self._validate_bounds_in_ui)
    ...
    def set_template_bounds(self, inner_margins: Margins, outer_margins: Margins | None = None) -> None:
        self.template.inner_margins_mm = inner_margins
        if outer_margins is not None:
            self.template.outer_margins_mm = outer_margins
        self._rebuild_bounds()
        self._schedule_bounds_validation()
    ...
    def addItem(self, item):
        ...
        if isinstance(item, BaseLabelItem):
            ...
            item.item_changed.connect(self._schedule_bounds_validation)
    ...
    def _schedule_bounds_validation(self) -> None:
        self._bounds_validation_timer.start()

    def configure_warning_settings(self, settings: AppSettings | None = None) -> None:
        self.settings = settings or AppSettings()
        for item in self.items():
            if isinstance(item, BaseLabelItem):
                item.set_warning_settings(self.settings)

    def _validate_bounds_in_ui(self) -> None:
        tolerance = self.settings.boundary_tolerance_mm
        for item in self.items():
            if not isinstance(item, BaseLabelItem):
                continue
            item_rect = item.mapToScene(item.boundingRect()).boundingRect()
            inner = self.inner_rect.adjusted(-tolerance * self.scale, -tolerance * self.scale, tolerance * self.scale, tolerance * self.scale)
            is_out = not inner.contains(item_rect)
            item.set_out_of_bounds(is_out and self.settings.boundary_warning_enabled)

        self.last_validation_report = [
            {"id": item.item_id, "reason": "outside_printable_area"}
            for item in self.items()
            if isinstance(item, BaseLabelItem)
            and self.settings.boundary_warning_enabled
            and not self.inner_rect.adjusted(
                -self.settings.boundary_tolerance_mm * self.scale,
                -self.settings.boundary_tolerance_mm * self.scale,
                self.settings.boundary_tolerance_mm * self.scale,
                self.settings.boundary_tolerance_mm * self.scale,
            ).contains(item.mapToScene(item.boundingRect()).boundingRect())
        ]
    ...
    def validate_item_bounds(self) -> List[BaseLabelItem]:
        """Validate printable bounds live and highlight the actual edited item."""
        self._validate_bounds_in_ui()
        return [
            item for item in self.items()
            if isinstance(item, BaseLabelItem) and item._is_out_of_bounds
        ]
